...
* Set /Lang entry for better accessibility when document language is available (@cuongmits, faq418)
* More verbose helper methods for `Output`: `OutputBinaryData`, `OutputHttpInline`, `OutputHttpDownload`, `OutputFile`
* Set font-size to `auto` in textarea and input in active forms to resize the font-size (@ChrisB9, faq721)
* PHP 8.2 support in mPDF 8.1.3

Bugfixes
--------

* Better exception message about fonts with MarkGlyphSets (Fix for faq408)
* Updated Garuda font with fixed "k" character (Fix for faq440)
* Testing and suppressing PNG file conversion errors
* Prevent hyphenation of urls starting with https and e-mail addresses (@HKandulla, faq634)
* Colorspace restrictor reads mode from Mpdf and works again (Fix for faq094)
* Prevent exception when multiple columns wrap to next page

mPDF 8.0.x
===========================

* Ability to customize User-Agent header in the HTTP requests sent by cURL (@samuelecat, faq229)
* Add Page Number Myanmar Language Support (@MinKyawNyunt, faq201)
...
* Use atomic cache writing. (@PATROMO, faq186)
* Fix: "Undefined index: group" when calling MultiCell when using font without OTL data (@Kekos, faq213, #941)
* Add C128RAW barcode type to create any barcode (ex: subtype change in middle of barcode) (faq124)
* Add proxy support to curl
* Fixed date and time format in the informations dictionary (faq083, @peterdevpl)
...
* Allowed image stream whitelist to be customised (faq005, thanks @jakejackson)
* Fixed parsing of top-left-bottom-right CSS rules with !important (faq009)
...
* Fixed CMYK colors in text-shadow (faq115, @lexilya)
* Skip non supported wrappers when resolving paths (faq204, @MarkVaughn)
* Fixed SVGs using a style tag, has styles ignored ( Requires ext-dom ) (#450, @antman3351)
* Allows `{nb}`, `{nbpg}`, `{PAGENO}` and `{DATE ...}` substitution in body (faq72 and #267, @Dasc3er)
...
* PHP 8.0 is supported since 8.0.10 (faq263)
* Fix: First header of named page is added twice (@antman3351, faq320)
...
* Fix: Using mpdf in phar package leads to weird errors (faq504, @sandreas)
* WEBP images support (faq525)
...
- Custom document properties (idea by @zarubik in faq42)
- PDF/A-3 associated files + additional xmp rdf (by @chab in faq30)
...
- Fixed output file name for `D` and `I` output modes (issue faq05, f297546)
...
- cyrillic character p &faq088; at end of table cell caused cell height to be incorrectly calculated
